...
assert.match(getCalendarNote(new Date(2026, 8, 2, 12)) ?? "", /Quốc khánh/, "Sep 2 is Quốc khánh");

// Payday (the 15th, an ordinary Tuesday).
assert.match(getCalendarNote(new Date(2026, 6, 15, 12)) ?? "", /lương/, "the 15th is payday");
