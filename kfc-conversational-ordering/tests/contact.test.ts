...
function orderFor(totalVnd: number, fulfillment: "delivery" | "pickup", address?: string): Order {
  ...
  return {
    ...
    quote: { fulfillment, address, deliveryFeeVnd: 15000, etaMinutes: 20, displayDeliveryFee: "15,000 VND" },
  };
}
...
assert.equal(
  canSkipOtp({ ...base, order: orderFor(150000, "delivery", "12 Main Street, District 1") }),
  false,
  "TRUSTED_SKIP_OTP unset ⇒ never skip",
);

// --- all conditions true => skip (normalized address match) -----------------

process.env.TRUSTED_SKIP_OTP = "1";
assert.equal(
  canSkipOtp({ ...base, order: orderFor(150000, "delivery", "12 Main Street, District 1") }),
  true,
  "trusted repeat, small order, matching address => skip",
);

// --- each condition individually false ⇒ no skip -----------------------------

assert.equal(
  canSkipOtp({ ...base, completedOrderCount: 0, order: orderFor(150000, "delivery", "12 Main Street, District 1") }),
  false,
  "zero completed orders ⇒ no skip",
);
assert.equal(
  canSkipOtp({ ...base, order: orderFor(200000, "delivery", "12 Main Street, District 1") }),
  false,
  "total at/above 200k ⇒ no skip",
);
assert.equal(
  canSkipOtp({ ...base, order: orderFor(150000, "delivery", "99 Other Street, District 7") }),
  false,
  "new/mismatched address ⇒ no skip",
);
assert.equal(
  canSkipOtp({ ...base, contact: null, order: orderFor(150000, "delivery", "12 Main Street, District 1") }),
  false,
...
assert.equal(
  canSkipOtp({
    ...
    order: orderFor(150000, "pickup", "District 1"),
  }),
  true,
